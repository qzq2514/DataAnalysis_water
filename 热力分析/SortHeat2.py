...
def handle(ReZhiBiao,Gongshui,HuiShui):   #理论流量计算公式中的一些参数
    ...
    print("SortHeat-handle%-----d %d %d",ReZhiBiao,Gongshui,HuiShui)
    path='orgData.xls'
    ...
    giveHeat=df['客户状态'].str.match('停热')    #用户状态为“供热”且瞬时流量为0,则“备注”列填充为“流量异常”
    ...
    dff=pd.DataFrame(np.random.rand(3,4),index=[1,2,3],columns=['A','B','C','D'])
